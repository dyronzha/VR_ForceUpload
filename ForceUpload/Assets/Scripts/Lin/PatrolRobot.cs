...
public class PatrolRobot : MonoBehaviour{
    ...
    void Start(){
        ...
        if (RouteType == 1){
            ...
            ray_Dir = new Ray(transform.position, new Vector3(3.0f, 0.0f, 0.0f));
            Debug.DrawRay(transform.position, new Vector3(3.0f, 0.0f, 0.0f));
        }
        else {
            Patrol_Dir = new Vector3(0.0f, 0.0f, 3.0f);
            ray_Dir = new Ray(transform.position, new Vector3(0.0f, 0.0f, 3.0f));
            Debug.DrawRay(transform.position, new Vector3(0.0f, 0.0f, 3.0f));
        }
    }
    ...
    void Update(){
        ...
        if (Physics.Raycast(ray_Dir, out hit_Dir, 3.0f)) {
            if (hit_Dir.transform.tag == "Stage3_Box"){
                if (RouteType == 1){
                    if (transform.position.x >= hit_Dir.transform.position.x) Ini_End = hit_Dir.transform.position + new Vector3(1.0f, 0.5f, 0.0f);
                    else Ini_End = hit_Dir.transform.position + new Vector3(-1.0f, 0.5f, 0.0f);
                }
                else if(RouteType == 2 && hit_Dir.transform.name == "Box_B"){
                    if (transform.position.z >= hit_Dir.transform.position.z) Ini_End = hit_Dir.transform.position + new Vector3(0.0f, 0.5f, 1.0f);
                    else Ini_End = hit_Dir.transform.position + new Vector3(0.0f, 0.5f, -1.0f);
                }
            }
    ...
    float CalculateNewSpeed() {
        ...
        if (tmp_dis == 0){
            ...
            if(RouteType ==1)Patrol_Dir.x = Patrol_Dir.x * -1.0f;
            else Patrol_Dir.z = Patrol_Dir.z * -1.0f;
        }
